...
{
	// cached errors are calculated for all data, not only for the current subset;
	// this is OK (generally), since the purpose of the subset is only to reduce computations
	...
	if (mep_parameters.get_problem_type() == MEP_PROBLEM_TIME_SERIE) {
		...
		data = training_data_ts.get_data_matrix_as_double();
	}
	else {
		num_rows = training_data.get_num_rows();
		data = training_data.get_data_matrix_as_double();
	}
	...
	if (mep_parameters.get_problem_type() == MEP_PROBLEM_REGRESSION ||
		...
		for (unsigned int v = 0; v < num_actual_variables; v++) {
			...
			if (mep_parameters.get_error_measure() == MEP_REGRESSION_MEAN_ABSOLUTE_ERROR)
				for (unsigned int k = 0; k < num_rows; k++) {
					cached_eval_variables_matrix_double[actual_variable][k] =
							data[k][actual_variable];
					for (unsigned int o = 0; o < num_problem_outputs; o++)
				...
				for (unsigned int k = 0; k < num_rows; k++) {
					...
					for (unsigned int o = 0; o < num_problem_outputs; o++)
						cached_sum_of_errors_for_variables[actual_variable][o] +=
							mep_squared_error_double(cached_eval_variables_matrix_double[actual_variable][k],
									data[k][num_total_variables + o]);
				}
			for (unsigned int o = 0; o < num_problem_outputs; o++)
		...
		for (unsigned int v = 0; v < num_actual_variables; v++) {
			...
			cached_sum_of_errors_for_variables[actual_enabled_variables[v]][0] = num_0_incorrect;
			...
			for (unsigned int i = 0; i < num_rows; i++) {
				...
				// check all between i and j and label them as belonging to class 0
				...
				if (num_0_incorrect + num_1_incorrect < cached_sum_of_errors_for_variables[actual_enabled_variables[v]][0]) {
					cached_sum_of_errors_for_variables[actual_enabled_variables[v]][0] = num_0_incorrect + num_1_incorrect;
					cached_threashold[actual_enabled_variables[v]] = array_value_class[i].value;
				}
				i = j;
				i--;
			}
			cached_sum_of_errors_for_variables[actual_enabled_variables[v]][0] /= (double)num_rows;
			cached_sum_of_errors_for_variables[actual_enabled_variables[v]][0] *= 100;
		}
	}
}
//---------------------------------------------------------------------------
void t_mep::allocate_values(double**** eval_double)
{
	unsigned int num_rows;
...
//---------------------------------------------------------------------------
void t_mep::delete_values(double**** eval_double)
{
	if (*eval_double) {
...
//---------------------------------------------------------------------------
void t_mep::compute_test_error(const t_mep_chromosome &best_program,
							   double *test_error_per_output,
							   double& total_test_error)
{
	unsigned int num_problem_outputs = mep_parameters.get_num_outputs();
	total_test_error = -1;
	for (unsigned int o = 0; o < num_problem_outputs; o++)
		test_error_per_output[o] = -1;
	...
	case MEP_PROBLEM_TIME_SERIE: {
		...
		if (mep_parameters.get_data_type() == MEP_DATA_LONG_LONG){
			long long* previous_data = new long long[window_size * mep_parameters.get_num_outputs()];

			compute_previous_data_for_time_series_test(previous_data);

			if (mep_parameters.get_error_measure() == MEP_REGRESSION_MEAN_ABSOLUTE_ERROR) {
				if (best_program.compute_time_series_error_on_data(
			...
			double* previous_data = new double[window_size * mep_parameters.get_num_outputs()];
		...
		break;
	case MEP_PROBLEM_BINARY_CLASSIFICATION:
		if (best_program.compute_binary_classification_error_on_double_data(
				test_data,
				total_test_error)) {
		}
		break;
	...
	case MEP_PROBLEM_MULTICLASS_CLASSIFICATION:
		switch (mep_parameters.get_error_measure()) {
		case MEP_MULTICLASS_CLASSIFICATION_WINNER_TAKES_ALL_ERROR:
			if (best_program.compute_multi_class_classification_winner_takes_all_fixed_error_on_double_data(
					test_data, total_test_error)) {
			}
			break;

		case MEP_MULTICLASS_CLASSIFICATION_SMOOTH_ERROR:
			if (best_program.compute_multi_class_classification_smooth_error_on_double_data(
					test_data,
					total_test_error)) {
			}
			break;
		case MEP_MULTICLASS_CLASSIFICATION_WINNER_TAKES_ALL_DYNAMIC_ERROR:
			if (best_program.compute_multi_class_classification_winner_takes_all_dynamic_error_on_double_data(
					test_data, total_test_error)) {
			}

			break;
		case MEP_MULTICLASS_CLASSIFICATION_CLOSEST_CENTER_ERROR:
			if (best_program.compute_multi_class_classification_closest_center_error_on_double_data(
				test_data, total_test_error)) {
			}
...
bool t_mep::predict_on_test(int run_index, double** output, char* valid_output)
{
	...
	double* previous_data = new double[window_size * num_outputs];
	//int num_previous_data;
	//double** previous_matrix;
	if (validation_data.get_num_rows()) {
		double** validation_matrix = validation_data.get_data_matrix_as_double();
		unsigned int num_validation_data = validation_data.get_num_rows();
		double** training_matrix = training_data.get_data_matrix_as_double();
...
bool t_mep::predict(int run_index, double** output, char* valid_output)
{
	...
	//double *tmp_out = new double[num_outputs];
	...
	for (unsigned int p = 0; p < num_predictions; p++) {
		if (get_output(run_index, previous_data, output[p])) {
			valid_output[p] = 1;
			for (unsigned int w = 0; w < (window_size - 1) * num_outputs; w++)
				previous_data[w] = previous_data[w + num_outputs];
			for (unsigned int c = 0; c < num_outputs; c++)
...
//---------------------------------------------------------------------------
void t_mep::compute_previous_data_for_time_series_validation(double *previous_data)
{
	unsigned int window_size = mep_parameters.get_window_size();
	unsigned int num_outputs = mep_parameters.get_num_outputs();
...
//---------------------------------------------------------------------------
void t_mep::compute_previous_data_for_time_series_training(double* previous_data)
{
	unsigned int window_size = mep_parameters.get_window_size();
	double** training_matrix = training_data.get_data_matrix_as_double();
	unsigned int num_outputs = mep_parameters.get_num_outputs();
...
//---------------------------------------------------------------------------
void t_mep::compute_previous_data_for_time_series_test(double* previous_data)
{
	unsigned int window_size = mep_parameters.get_window_size();
...
//---------------------------------------------------------------------------
void t_mep::compute_previous_data_for_time_series_prediction(double* previous_data)
{
	unsigned int window_size = mep_parameters.get_window_size();
	unsigned int num_outputs = mep_parameters.get_num_outputs();

	if (test_data.get_num_rows()) {
		double** test_matrix = test_data.get_data_matrix_as_double();
		unsigned int num_test_data = test_data.get_num_rows();
																	 ...
																	 double* max_data)
{
	...
	if (get_training) {
		...
		compute_output_on_training(run_index, output + num_data, valid_output + num_data);
													   ...
													   double* max_data)
{
	...
	unsigned int window_size = mep_parameters.get_window_size();
	double *prev_data = new double[window_size];

	if (get_training) {

		unsigned int local_num_data = training_data.get_num_rows();
		compute_output_on_training(run_index, output + num_data,
...
void t_mep::compute_output_on_all_data(bool get_training,
									   ...
									   double** output,
									   char* valid_output,
										double* min_data, double* max_data)
{
	if (mep_parameters.get_problem_type() == MEP_PROBLEM_TIME_SERIE)
										   ...
										   double *max_data)
{
	...
	if (get_training) {
		...
		for (unsigned int i = 0; i < local_num_data; i++) {
			mep_data_ptr->get_range_values_double(i, num_cols - num_outputs, num_outputs,
												  target[num_data + i]);
	...
	if (get_validation) {
		...
		for (unsigned int i = 0; i < local_num_data; i++) {
			mep_data_ptr->get_range_values_double(i, num_cols - num_outputs, num_outputs,
												  target[num_data + i]);			has_target[num_data + i] = 1;
	...
	switch (mep_parameters.get_problem_type()) {
		...
		case MEP_PROBLEM_MULTICLASS_CLASSIFICATION:
		{
			...
			for (unsigned int i = 0; i < num_data; i++) {
				valid_output[i] = get_output(run_index,
											 training_data.get_row_as_double(i),
											 output[i]);
			}
		}
			break;
		case MEP_PROBLEM_TIME_SERIE:
		{
			...
			statistics.get_stat_ptr(run_index)->best_program.compute_time_series_output_on_given_data(
				previous_data, window_size,
				training_matrix + window_size, num_training_data - window_size,
...
//---------------------------------------------------------------------------
void t_mep::compute_output_on_validation(int run_index,
										 double** output,
										 char* valid_output)
{
	if (run_index == -1)
	...
	case MEP_PROBLEM_MULTICLASS_CLASSIFICATION:
	{
		...
		for (unsigned int i = 0; i < num_data; i++) {
			valid_output[i] = get_output(run_index,
										 validation_data.get_row_as_double(i),
										 output[i]);
		}
	}
		break;
