	...
	t_seed& seed)
{
	...
	compute_eval_matrix(num_rows, cached_variables_eval_matrix,
		num_actual_variables, actual_enabled_variables,
						line_of_constants, eval_matrix_double, seed);

	double best_threshold;
	for (unsigned int i = 0; i < code_length; i++) {   // parse the t_mep_chromosome from top to down
		double sum_of_errors;
		if (prg[i].op >= 0)// a variable
			if (prg[i].op < (int)num_total_variables) { // a variable, which is cached already
				sum_of_errors = cached_sum_of_errors[prg[i].op][0];
			...
			for (unsigned int k = 0; k < random_subset_selection_size; k++) {
				...
				tmp_value_class[k].class_index = class_index[random_subset_indexes[k]];
				if (tmp_value_class[k].class_index == 0)
					num_0_incorrect++;
			}
			...
			for (unsigned int t = 0; t < random_subset_selection_size; t++) {
				...
				while (j < random_subset_selection_size && 
					fabs(tmp_value_class[t].value - tmp_value_class[j].value) < 1e-6)// skip all equals to find the threshold.
					j++;
				...
				for (unsigned int k = t; k < j; k++)
					if (tmp_value_class[k].class_index == 0)
						...
						if (tmp_value_class[k].class_index == 1) {
							//num_0_incorrect--;
							num_1_incorrect++;
						}
					}
...
bool t_mep_chromosome::compute_binary_classification_error_on_double_data_return_error(
    ...
    double& error, unsigned int& index_error_gene) const
{
	error = 0;
	double actual_output_double[1];
