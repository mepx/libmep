...
int t_mep_parameters::from_xml(pugi::xml_node parent)
{
	...
	if (node) {
		...
		case MEP_PROBLEM_BINARY_CLASSIFICATION:
			if (value == MEP_BINARY_CLASSIFICATION_AUTOMATIC_THRESHOLD)
				error_measure = value;
			else
				error_measure = MEP_BINARY_CLASSIFICATION_AUTOMATIC_THRESHOLD;
			break;
		case MEP_PROBLEM_MULTICLASS_CLASSIFICATION:
			if (value == MEP_MULTICLASS_CLASSIFICATION_WINNER_TAKES_ALL_ERROR || 
				value == MEP_MULTICLASS_CLASSIFICATION_SMOOTH_ERROR || 
				value == MEP_MULTICLASS_CLASSIFICATION_WINNER_TAKES_ALL_DYNAMIC_ERROR ||
				value == MEP_MULTICLASS_CLASSIFICATION_CLOSEST_CENTER_ERROR
				)
				error_measure = value;
			else
