...
void t_mep::compute_cached_eval_matrix_double2(s_value_class *array_value_class)
{
	if (parameters.problem_type == PROBLEM_REGRESSION)
		...
		for (int v = 0; v < num_actual_variables; v++) {
			...
			cached_threashold[actual_used_variables[v]] = 0;
			for (int k = 0; k < training_data.num_data; k++) {
				cached_eval_matrix_double[actual_used_variables[v]][k] = training_data._data_double[k][actual_used_variables[v]];
			...
			int num_0_incorrect = training_data.num_class_0;
			int num_1_incorrect = 0;
			cached_threashold[actual_used_variables[v]] = array_value_class[0].value - 1;// all are classified to class 1 in this case
			cached_sum_of_errors[actual_used_variables[v]] = num_0_incorrect;
...
int t_mep::start(f_on_progress on_generation, f_on_progress on_new_evaluation, f_on_progress on_complete_run)
{
	...
	stats = new t_mep_stat[parameters.num_runs];
	...
	while (*current_subpop_index < parameters.num_subpopulations) {
		...
		if (generation_index == 0) {
			...
			for (int k = 0; k < parameters.subpopulation_size; k += 2) {
				...
				a_sub_population->offspring1.mutation(&parameters, actual_operators, num_operators, actual_used_variables, num_actual_variables);
				if (parameters.problem_type == PROBLEM_REGRESSION)
				...
				a_sub_population->offspring2.mutation(&parameters, actual_operators, num_operators, actual_used_variables, num_actual_variables);
				if (parameters.problem_type == PROBLEM_REGRESSION)
...
bool t_mep::start_steady_state(int run, double ***eval_double, s_value_class **array_value_class, f_on_progress on_generation, f_on_progress on_new_evaluation)       // Steady-State MEP
{
	...
	*/
	...
	if (test_data.num_data && test_data.num_targets) {// has target
...
int t_mep::from_pugixml_node(pugi::xml_node parent)
{
	...
	*/
	...
	if (last_run_index > -1) {
		stats = new t_mep_stat[last_run_index + 1];
		last_run_index = 0;
		for (pugi::xml_node row = node_results.child("run"); row; row = row.next_sibling("run"), last_run_index++)
...
{
	if (((t_mep_stat*)a)->running_time < ((t_mep_stat*)b)->running_time)
		return -1;
	else
		if (((t_mep_stat*)a)->running_time > ((t_mep_stat*)b)->running_time)
			return 1;
...
{
	if (ascending)
		qsort((void*)stats, last_run_index + 1, sizeof(t_mep_stat), running_time_comparator_ascending);
	else
		qsort((void*)stats, last_run_index + 1, sizeof(t_mep_stat), running_time_comparator_descending);
}
//---------------------------------------------------------------------------
int training_error_comparator_ascending(const void * a, const void * b)
{
	if (((t_mep_stat*)a)->best_training_error[((t_mep_stat*)a)->last_gen] < ((t_mep_stat*)b)->best_training_error[((t_mep_stat*)b)->last_gen])
		return -1;
	else
		if (((t_mep_stat*)a)->best_training_error[((t_mep_stat*)a)->last_gen] > ((t_mep_stat*)b)->best_training_error[((t_mep_stat*)b)->last_gen])
			return 1;
...
{
	if (ascending)
		qsort((void*)stats, last_run_index + 1, sizeof(t_mep_stat), training_error_comparator_ascending);
	else
		qsort((void*)stats, last_run_index + 1, sizeof(t_mep_stat), training_error_comparator_descending);

}
//---------------------------------------------------------------------------
int validation_error_comparator_ascending(const void * a, const void * b)
{
	if (((t_mep_stat*)a)->best_validation_error < ((t_mep_stat*)b)->best_validation_error)
		return -1;
	else
		if (((t_mep_stat*)a)->best_validation_error > ((t_mep_stat*)b)->best_validation_error)
			return 1;
...
{
	if (ascending)
		qsort((void*)stats, last_run_index + 1, sizeof(t_mep_stat), validation_error_comparator_ascending);
	else
		qsort((void*)stats, last_run_index + 1, sizeof(t_mep_stat), validation_error_comparator_descending);

}
//---------------------------------------------------------------------------
int test_error_comparator_ascending(const void * a, const void * b)
{
	if (((t_mep_stat*)a)->test_error < ((t_mep_stat*)b)->test_error)
		return -1;
	else
		if (((t_mep_stat*)a)->test_error > ((t_mep_stat*)b)->test_error)
			return 1;
...
{
	if (ascending)
		qsort((void*)stats, last_run_index + 1, sizeof(t_mep_stat), test_error_comparator_ascending);
	else
		qsort((void*)stats, last_run_index + 1, sizeof(t_mep_stat), test_error_comparator_descending);
}
//---------------------------------------------------------------------------
void t_mep::get_list_of_used_variables(void)
{
	if (actual_used_variables) {
		delete[](actual_used_variables);
		(actual_used_variables) = NULL;
...
{
	return training_data.to_csv(file_name, list_separator);
}
//---------------------------------------------------------------------------
int t_mep::get_training_data_type(void)
{
	return training_data.data_type;
}
//---------------------------------------------------------------------------
int t_mep::get_training_data_num_columns(void)
{
	return training_data.num_cols;
}
//---------------------------------------------------------------------------
int t_mep::get_training_data_num_rows(void)
{
	return training_data.num_data;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
char *t_mep::get_training_data_as_string(int row, int col)
{
	return training_data._data_string[row][col];
}
//---------------------------------------------------------------------------
double t_mep::get_training_data_as_double(int row, int col)
{
...
{
	return training_data.find_symbol_from_selected_col(find_what, col, use_regular);
}
//---------------------------------------------------------------------------
int t_mep::training_data_find_symbol_from_all_variables(const char *find_what, bool use_regular)
{
	return training_data.find_symbol_from_all_variables(find_what, use_regular);
}
//---------------------------------------------------------------------------
