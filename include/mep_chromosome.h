...
private:
	...
	void compute_eval_matrix_double(int num_training_data, double **cached_eval_matrix_double, int num_actual_variables, int * actual_enabled_variables, int *line_of_constants, double ** eval_double, t_seed& seed);
	

	void fitness_regression_double_cache_all_training_data(const t_mep_data &mep_dataset, int *random_subset_indexes, int random_subset_selection_size, double** cached_eval_matrix, double * cached_sum_of_errors, int num_actual_variables, int * actual_enabled_variables, double **, t_mep_error_function, t_seed &seed);
	void fitness_regression_double_no_cache(const t_mep_data &mep_dataset, int *random_subset_indexes, int random_subset_selection_size, double* eval_vect, double *sum_of_errors_array, t_mep_error_function);
	...
	int *index_best_genes;          // the index of the best expression in chromosome
	int max_index_best_genes;       // index of the max
...
public:
	...
	char * to_C_double(bool simplified, double *data, int problem_type, int error_measure, int num_classes);
	char* to_Excel_function_double(bool simplified, double* data, int problem_type, int error_measure, int num_classes);
	...
	void allocate_memory(long code_length, int num_total_vars, bool use_constants, t_mep_constants *constants, int num_outputs);
	...
	void generate_random(const t_mep_parameters &parameters, const t_mep_constants & mep_constants, 
		...
		const int *actual_used_variables, int num_actual_used_variables, t_seed& seed);
    
	int compare(const t_mep_chromosome &other, bool minimize_operations_count);

	void mutation(const t_mep_parameters &parameters, 
			const t_mep_constants & mep_constants, const int *actual_operators, int num_actual_operators, 
		const int *actual_used_variables, int num_actual_used_variables, t_seed& seed);
	...
	void fitness_regression(const t_mep_data &mep_dataset, int *random_subset_indexes, int random_subset_selection_size, double** cached_eval_matrix, double * cached_sum_of_errors, int num_actual_variables, int * actual_enabled_variables, double **, t_mep_error_function, t_seed &seed);

	void fitness_binary_classification(const t_mep_data &mep_dataset, int *random_subset_indexes, int random_subset_selection_size, double **cached_eval_matrix, double * cached_sum_of_errors, double * cached_threashold, int num_actual_variables, int * actual_enabled_variables, double **eval_matrix_double, s_value_class *tmp_value_class, t_seed &seed);
	void fitness_multi_class_classification_smooth(const t_mep_data &mep_dataset, int *random_subset_indexes, int random_subset_selection_size, double **cached_eval_matrix, int num_actual_variables, int * actual_enabled_variables, double **eval_matrix_double, t_seed &seed);
	void fitness_multi_class_classification_winner_takes_all_fixed(const t_mep_data &mep_dataset, int *random_subset_indexes, int random_subset_selection_size, double **cached_eval_matrix, int num_actual_variables, int * actual_enabled_variables, double **eval_matrix_double, t_seed &seed);
	void fitness_multi_class_classification_winner_takes_all_dynamic(const t_mep_data &mep_dataset, int *random_subset_indexes, int random_subset_selection_size, double **cached_eval_matrix, int num_actual_variables, int * actual_enabled_variables, double **eval_matrix_double, t_seed &seed);
	...
	bool compute_multi_class_classification_error_on_double_data_return_error(double **inputs, int num_data, int output_col, int num_classes, double &error, int &index_error_gene, double &num_incorrectly_classified);
	bool compute_multi_class_classification_winner_takes_all_dynamic_error_on_double_data_return_error(double **data, int num_data, int output_col, int num_classes, double &error, int &index_error_gene, double &_num_incorrectly_classified);
	bool compute_multi_class_classification_winner_takes_all_fixed_error_on_double_data(double **data, int num_data, int output_col, int num_classes, double &error);
	bool compute_multi_class_classification_smooth_error_on_double_data(double **data, int num_data, int output_col, int num_classes, double &error);
	bool compute_multi_class_classification_winner_takes_all_dynamic_error_on_double_data(double **data, int num_data, int output_col, int num_classes, double &error);
	...
	int get_index_best_gene(int output_index);
